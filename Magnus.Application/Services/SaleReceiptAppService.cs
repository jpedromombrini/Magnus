...
public class SaleReceiptAppService(
    ...
    ISaleService saleService) : ISaleReceiptAppService
{
    public async Task<IEnumerable<SaleReceiptResponse>> GetSaleReceiptsAsync(CancellationToken cancellationToken)
    {
        return mapper.Map<IEnumerable<SaleReceiptResponse>>(await unitOfWork.SaleReceipts.GetAllAsync(cancellationToken));
    }

    public async Task<IEnumerable<SaleReceiptResponse>> GetSaleReceiptsByFilterAsync(GetSaleReceiptFilter filter, CancellationToken cancellationToken)
    {
        return mapper.Map<IEnumerable<SaleReceiptResponse>>(await unitOfWork.SaleReceipts.GetAllByExpressionAsync(
            x =>
                ...
                (x.Installments.Any(y => y.DueDate >= filter.InitialDate && y.DueDate <= filter.FinalDate)), cancellationToken));
    }
    ...
    public async Task<IEnumerable<SaleReceiptResponse>> ListSaleReceiptByIdAsync(Guid saleId, CancellationToken cancellationToken)
    {
        return (await unitOfWork.SaleReceipts.GetAllByExpressionAsync(x => x.SaleId == saleId, cancellationToken)).MapToResponse();
    }
