...
public class SupplierService(IUnitOfWork unitOfWork) : ISupplierService
{
    public async Task AddSupplierAsync(Supplier supplier, CancellationToken cancellationToken)
    {
        ...
        if (supplierDb is not null)
            throw new BusinessRuleException("Já existe um fornecedor com esse documento");
        supplierDb =
            await unitOfWork.Suppliers.GetByExpressionAsync(
                x => supplier.Email != null && x.Email.Address.ToLower() == supplier.Email.Address.ToLower(),
                cancellationToken);
        if (supplierDb is not null)
            throw new BusinessRuleException("Já existe um fornecedor com esse email");
        supplierDb = await unitOfWork.Suppliers.GetByExpressionAsync(
            x => x.Name.ToLower() == supplier.Name.ToLower(), cancellationToken);
        if (supplierDb is not null)
            throw new BusinessRuleException("Já existe um fornecedor com esse nome");
